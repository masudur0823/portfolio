      ...
      <Container>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-x-5 gap-y-10 ">
          {works?.map((project, index) => (
            <div
              key={index}
              className="hover:shadow-lg rounded-lg text-center pt-2 pb-7"
            >
              ...
              <p className="text-gray-500 mt-4">{project?.desc}</p>

              <p className="text-gray-500 mb-4">Tools: {project?.tools}</p>
              <button className="bg-cyan-500 text-white md:px-8 px-5 md:py-2 py-1 rounded-md font-semibold">
                View Site
              </button>
            </div>
          ))}
